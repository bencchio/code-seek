...
fn filter_combined_match_and_depth() {
    ...
    assert!(
        ...
        "non-matching entity must be absent"
    );
    assert!(
...
fn filter_empty_match_shows_file_header() {
    ...
    assert!(
        ...
        "file header must still appear with 0 matches"
    );
    assert!(stdout.contains("0 entities"), "entity count must be 0");
    assert!(
        !stdout.contains("add"),
        "non-matching entity must not appear"
    );
...
fn filter_match_preserves_parent_impl() {
    ...
    assert!(
        !stdout.contains("baz"),
        "non-matching child must be excluded"
    );
...
fn filter_max_depth_truncates_children() {
    ...
    let _ = fs::remove_dir_all(&dir);
    fs::create_dir_all(&dir).unwrap();
    fs::write(dir.join("foo.rs"), "impl Foo {\n    fn bar(&self) {}\n}\n").unwrap();
...
fn scan_empty_rs_file() {
    ...
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains("0 entities"),
        "empty file must show 0 entities"
    );
...
#[test]
fn scan_elixir_file() {
    let dir = std::env::temp_dir().join("code_seek_inttest_elixir");
    let _ = fs::remove_dir_all(&dir);
    fs::create_dir_all(&dir).unwrap();
    fs::write(
        dir.join("foo.ex"),
    ...
    let out = command()
        .args(["scan", dir.to_str().unwrap(), "--lang", "elixir"])
        .output()
        .unwrap();
    assert!(
        out.status.success(),
...
fn scan_info_no_tests_excludes_test_module() {
    ...
    assert_eq!(
        json["total_entities"].as_u64().unwrap(),
        1,
        "no-tests should exclude test mod"
    );
...
fn scan_info_tests_only_shows_test_module() {
    ...
    assert_eq!(
        json["total_entities"].as_u64().unwrap(),
        2,
        "tests-only should keep mod + child"
    );
...
fn scan_invalid_lang_cli() {
    ...
    assert!(
        ...
        "error must mention the invalid lang"
    );
...
fn scan_json_dependency_kind_internal() {
    ...
    let main_file = json["files"]
        .as_array()
        .unwrap()
        .iter()
        .find(|f| f["path"].as_str().unwrap().ends_with("main.rs"))
        .unwrap();
...
fn scan_json_entity_paths_are_correct() {
    ...
    assert!(
        method_path.ends_with("> impl Foo > bar"),
        "got: {method_path}"
    );
...
fn scan_json_includes_dependencies_field() {
    ...
    let main_file = json["files"]
        ...
        .unwrap();
    assert!(
        main_file.get("dependencies").is_some(),
        "dependencies field missing"
    );
    let deps = main_file["dependencies"].as_array().unwrap();
    ...
    let helper_file = json["files"]
        .as_array()
        .unwrap()
        .iter()
        .find(|f| f["path"].as_str().unwrap().ends_with("helper.rs"))
        .unwrap();
    ...
    let stdout_tree = String::from_utf8_lossy(&out_tree.stdout);
    assert!(
        stdout_tree.contains("deps"),
        "tree output should mention dependencies"
    );
...
fn scan_json_includes_errors_field() {
    ...
    assert_eq!(
        json["total_errors"].as_u64().unwrap(),
        1,
        "total_errors should be exactly 1"
    );
    ...
    let e = &errors[0];
    assert!(
        ["error", "missing"].contains(&e["kind"].as_str().unwrap()),
        "kind must be error or missing"
    );
    assert!(
        e["node_kind"].as_str().is_some(),
        "node_kind must be a string"
    );
    assert!(
        e["start_line"].as_u64().is_some(),
        "start_line must be an integer"
    );
    assert!(
        e["end_line"].as_u64().is_some(),
        "end_line must be an integer"
    );
    assert!(
        e["start_line"].as_u64().unwrap() >= 1,
    ...
    let ok = json["files"]
        ...
        .unwrap();
    assert_eq!(
        ok["errors"].as_array().unwrap().len(),
        0,
        "ok.rs should have no errors"
    );
    ...
    let stdout_tree = String::from_utf8_lossy(&out_tree.stdout);
    assert!(
        stdout_tree.contains("1 error"),
        "bad.rs header should show '1 error'"
    );
    assert!(
        stdout_tree.contains('⚠'),
        "bad.rs entity line should have ⚠ marker"
    );
    assert!(
        stdout_tree.contains('✓'),
        "ok.rs entity line should have ✓ marker"
    );
...
fn scan_nonexistent_path() {
    let out = command()
        .args(["scan", "/nonexistent_code_seek_path"])
        .output()
        .unwrap();
    assert!(!out.status.success());
...
fn scan_only_comments_go() {
    ...
    assert!(
        stdout.contains("0 entities"),
        "comment-only Go must show 0 entities"
    );
...
fn scan_only_comments_rs() {
    ...
    assert!(
        stdout.contains("0 entities"),
        "comment-only file must show 0 entities"
    );
...
fn scan_symlink_root_no_follow() {
    ...
    assert!(
        ...
        "symlink root without follow must show 0 files"
    );
...
fn scan_uses_cache_on_second_run() {
    ...
    let cache_path = std::env::temp_dir()
        .join("code_seek_test_xdg_state/code-seek")
        .join(dir.file_name().unwrap())
        .join("cache.json");
    assert!(
        cache_path.exists(),
        "cache.json must be created after first scan"
    );
    ...
    assert_eq!(
        stdout1, stdout2,
        "cached scan must produce identical output"
    );
...
fn scan_with_ignore_flag_skips_dirs() {
    ...
    assert!(
        !stdout.contains("build.rs"),
        "target/debug/build.rs must be skipped"
    );
...
#[test]
fn scan_hides_files_git_ignores() {
    let dir = std::env::temp_dir().join("code_seek_test_gitignore");
    let _ = fs::remove_dir_all(&dir);
    ...
    Command::new("git")
        ...
        .output()
        .unwrap();
    fs::write(dir.join(".gitignore"), b"vendored/\nsecret.rs\n").unwrap();
    fs::write(dir.join("shown.rs"), b"fn shown() {}").unwrap();
    fs::write(dir.join("secret.rs"), b"fn secret() {}").unwrap();
    fs::write(dir.join("vendored/dep.rs"), b"fn dep() {}").unwrap();
    ...
    assert!(out.status.success());
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(stdout.contains("shown.rs"), "tracked source must appear");
    assert!(!stdout.contains("secret.rs"), "ignored file must be hidden");
    assert!(
        !stdout.contains("dep.rs"),
        "ignored directory must be hidden"
    );

    let out = command()
        .args(["scan", dir.to_str().unwrap(), "--no-gitignore"])
        .output()
        .unwrap();
    assert!(out.status.success());
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(stdout.contains("secret.rs"), "--no-gitignore must show it");
    assert!(stdout.contains("dep.rs"), "--no-gitignore must show it");

    fs::remove_dir_all(&dir).unwrap();
}

#[test]
fn scan_outside_a_git_repository_is_unchanged() {
    let dir = std::env::temp_dir().join("code_seek_test_no_repo");
    let _ = fs::remove_dir_all(&dir);
    fs::create_dir_all(&dir).unwrap();
    fs::write(dir.join(".gitignore"), b"ignored.rs\n").unwrap();
    fs::write(dir.join("ignored.rs"), b"fn ignored() {}").unwrap();
    ...
    assert!(
        ...
        "with no repository there is no verdict to honor"
    );
